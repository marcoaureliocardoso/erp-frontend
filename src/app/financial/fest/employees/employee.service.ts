...
function matches(employee: Employee, term: string) {
  return (
    ...
    employee.education.toLowerCase().includes(term.toLowerCase()) ||
...
export class EmployeeService {
  ...
  private _search(): Observable<SearchResult> {
    return new Observable<SearchResult>((observer) => {
      this.http.get<Employee[]>(API_FINANCIAL_FEST + '/employees').subscribe({
        next: (data) => {
  ...
  public getEmployees(): Observable<Employee[]> {
    return this.http.get<Employee[]>(API_FINANCIAL_FEST + '/employees');
  }

  public getEmployee(id: number): Observable<Employee | undefined> {
    return this.http.get<Employee>(API_FINANCIAL_FEST + '/employees/' + id);
  }

  public create(employee: Employee): Observable<Employee> {
    return this.http.post<Employee>(API_FINANCIAL_FEST + '/employees', employee);
  }

  public update(employee: Employee): Observable<Employee> {
    return this.http.put<Employee>(API_FINANCIAL_FEST + '/employees/' + employee.id, employee);
  }
}
